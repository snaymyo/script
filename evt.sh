...
display_user_table() {
    ...
    while IFS=: read -r username _ _ _ _ _ shell; do
        if [[ "$shell" == "/bin/false" ]]; then
            ...
            if netstat -tnp 2>/dev/null | grep sshd | grep ESTABLISHED | grep -w "$username" &>/dev/null; then
                status_fmt="${GREEN}Online${NC}"
                padding="      " # 6 spaces for Online
            else
                status_fmt="${RED}Offline${NC}"
                padding="     " # 5 spaces for Offline
            fi
            
            printf " ${CYAN}│${NC} %-15s ${CYAN}│${NC} %-10s ${CYAN}│${NC} %s%s ${CYAN}│${NC} %-26s ${CYAN}│${NC}\n" "$username" "$pass_find" "$status_fmt" "$padding" "$exp"
        fi
    done < /etc/passwd
    echo -e "${CYAN}└─────────────────┴────────────┴──────────────┴────────────────────────────┘${NC}"
    echo ""
...
# --- SHORTCUT SETUP (evt command) ---
SCRIPT_PATH=$(readlink -f "$0")
if [[ "$SCRIPT_PATH" != "/usr/local/bin/evt" ]]; then
    ln -sf "$SCRIPT_PATH" /usr/local/bin/evt
...
display_user_table() {
    ...
    while IFS=: read -r username _ _ _ _ _ shell; do
        if [[ "$shell" == "/bin/false" ]]; then
            ...
            printf " ${CYAN}│${NC} %-15s ${CYAN}│${NC} %-10s ${CYAN}│${NC} %s%s ${CYAN}│${NC} %-26s ${CYAN}│${NC}\n" "$username" "$pass_find" "$status_fmt" "$pad" "$exp"
        fi
    done < /etc/passwd
    echo -e "${CYAN}└─────────────────┴────────────┴──────────────┴────────────────────────────┘${NC}"
    echo ""
